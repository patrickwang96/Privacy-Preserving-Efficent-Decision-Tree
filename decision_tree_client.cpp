...
void
carry_calculation_client_batch_compressed(int G_star[], int P_star[], int G1[], int P1[], int G2[], int P2[], int m,
                                          const triplet_b &tri_b, NetAdapter *net) {
    int *gp = new int[m * 2];
    auto pp = gp + m;
    auto b1 = new int[m * 2];
    ...
    secure_mul_client_batch_compressed(b1, b2, gp, m * 2, tri_b, net);
    for (int i = 0; i < m; i++) {
        G_star[i] = G1[i] + gp[i];
        P_star[i] = pp[i];
    }
